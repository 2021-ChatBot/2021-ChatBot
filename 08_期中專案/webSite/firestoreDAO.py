...
class Firestore:
    ...
    def updateMember(self, data):
        ...
        doc = self.__db.document(f"members/{data['id']}")
        doc.update(data)
    ...
    def setFootprint(self, data):
        ...
        data['id'] = footprint_ref.add(data)[1].id
        footprint_ref.document(data['id']).update(data)
        site_ref = self.__db.collection(f"companies/{data['companyId']}/{self.siteTable}/{data['siteId']}/footprints/{data['id']}")
        site_ref.set(data)

    def getFootprints(self, data):
        footprints = []
        if "infectedTime" in data.keys():
            # data{companyId,siteId,infectedTime,strength}
