...
class User(db.Model, UserMixin):
    id = db.Column(db.Integer, primary_key=True)
    lineId = db.Column(db.String(255), unique=True)
...
def signUp():
    if request.method == 'GET':
        ...
        emailQuery = User.query.filter_by(email=userData['email']).first()
        lineQuery = User.query.filter_by(lineId=userData['lineId']).first()

        if emailQuery == None:
            if userData['lineId'] == "" or lineQuery == None:
            ...
            return render_template("signUpFail.html", title="註冊失敗 本信箱(" + str(emailQuery.email) + ")已由他人註冊")
...
def infectedFootprints():
    ...
    event['eventId'] = requestAPI("POST", "/event", event)['eventId']
